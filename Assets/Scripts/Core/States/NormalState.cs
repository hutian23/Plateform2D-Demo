    ...
    public class NormalState : BaseActionState
    {
        ...
        public override EActionState Update(float deltaTime)
        {
            ...
            else
            {
                ...
                {
                    //对应了 dash ---> run这个过程，DashSpeed要减速到MaxRun
            ...
            //计算竖直速度
            {
                ...
                {
                    this.ctx.MaxFall = Mathf.MoveTowards(ctx.MaxFall, maxFallSpeed, Constants.FastMaxAccel * deltaTime);
                }
            }
            ...
            if (!ctx.OnGround)
            {
                ...
                //朝向相同，或者按住J都可以进入WallSlide
                if ((ctx.MoveX == (int)ctx.Facing) || (ctx.MoveX == 0 && GameInput.Grab.Checked()) && ctx.MoveY != -1)
                {
                    //判断是否向下做Wall滑行
                    if (ctx.Speed.y <= 0 && ctx.WallSlideTimer > 0 && ctx.CollideCheck(ctx.Position, Vector2.right * (int)ctx.Facing) && ctx.CanUnDuck)
                    {
                        ctx.Ducking = false;
